...
//MapString returns map marker as string format
func MapString(i int) string {
	switch i {
	case Sea:
		return "Sea"
	case Hitted:
...
func (m *Map) MakePadding(boat BoatStruct) {
	...
	for j := boat.LeftTopPoint.Y - 1; j < boat.LeftTopPoint.Y+1+boat.Size; j++ {
		for i := boat.LeftTopPoint.X - 1; i <= boat.LeftTopPoint.X+1; i++ {
			...
			if (j >= boat.LeftTopPoint.Y) && (j <= boat.LeftTopPoint.Y+boat.Size-1) &&
				(i == boat.LeftTopPoint.X) {
				continue
			}
...
func (m *Map) IsBoatZone(boat BoatStruct, x int, y int) bool {
	...
	if boat.IsHorizontal {
		if (x >= boat.LeftTopPoint.X) && (x <= boat.LeftTopPoint.X+boat.Size-1) &&
	...
	// Is vertical
	if (y >= boat.LeftTopPoint.Y) && (y <= boat.LeftTopPoint.Y+boat.Size-1) &&
...
func (m *Map) FindBoat(x, y int) (b BoatStruct) {
	...
	return BoatStruct{false, Point{-1, -1}, 0, false}
}
...
//IsBoatKilled Map
func (m *Map) IsBoatKilled(x, y int) bool {
	boat := m.FindBoat(x, y)
	return m.IsBoatDead(boat)
}
...
func PrintMaps(m1Own Map, m1Enemy Map, m2Own Map, m2Enemy Map) {
	//TODO: писать названия карты из мапы
	log.Println("Player1Own    Player1Enemy    Player2Own    Player2Enemy")
	log.Println(" |ABCDEFGHIJ   |ABCDEFGHIJ     |ABCDEFGHIJ   |ABCDEFGHIJ")
	log.Println("------------  ------------    ------------  ------------")
	for j := 0; j < 10; j++ {
		s1 := strconv.Itoa(j) + "|"
		s2 := strconv.Itoa(j) + "|"
...
//IsAllBoatsKilled Map
func (m *Map) IsAllBoatsKilled() bool {
	for _, boat := range m.Boats.List {
